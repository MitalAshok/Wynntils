...
public class ToastOverlay extends Overlay {
    ...
    public void tick(TickEvent.ClientTickEvent event, long ticks) {
        if (Minecraft.getMinecraft().currentScreen != null) return; // HeyZeer0: This will avoid toasts being processed when the user can't view
                                                                    // them
        ...
        if (OverlayConfig.ToastsSettings.INSTANCE.enableToast) {
            ...
            for (int j = 0; j < DISPLAY_AMNT; j++) {
                ...
                if (displayedToast[j] == null) continue;
                if ((displayedToast[j].getAnimated() > 160 || displayedToast[j].getAnimated() < -160) && (Minecraft.getSystemTime() - displayedToast[j].getCreationTime()) > 5000L) toBeRemoved.add(j);
            }
